...
static bool pauseStream = false;
static bool stopStream = false;
static bool motionDetected = false;

void* cameraThread(void* _);
...
// double readingToVoltage(int reading)
// {
...
void* cameraThread(void* _){
        ...
        while(!stopStream){
	        ...
	        count = frame_count;
                ...
                while (((count-- > 0) || loopIsInfinite) && !pauseStream && !stopStream) {
                        for (;;) {
                                fd_set fds;
                                struct timeval tv;
                                int r;
                        ...
                        //if motion not detected for the first time, set motionDetected to false
                        if (getMotionValue() == 0 && motionDetected == true){
                                motionDetected = false;
                                lastMotionTime = getTimeInMs();
                                printf("\nNo Motion Detected: Stream will pause in 6 seconds\n");
                                //displayMode(0);
                               
                        } 
                        
                        //if motionDetected, set motionDetected to true and reset lastMotionTime
                        if (getMotionValue() == 1) {
                                motionDetected = true;
                                lastMotionTime = getTimeInMs();
                                displayMode(1);
                        ...
                        //if !motionDetected for x number of seconds, pause streaming
                        if (!motionDetected && getTimeInMs() > lastMotionTime + 6*1000){
                                
                                printf("\nNo Motion Detected for 6 seconds:Pausing Stream\n");
                                pauseStream = true;
                                displayMode(0);
                        }
                        //displayMode(1);
                ...
                while(pauseStream && !stopStream){
                        ...
                        //resume stream if motion detected
                        if (getMotionValue() == 1){
                                printf("\nResumed Stream\n");
                                pauseStream = false;
                                displayMode(1);
...
void startCameraThread(){
        pthread_create(&cameraId, NULL, &cameraThread, NULL);
}
